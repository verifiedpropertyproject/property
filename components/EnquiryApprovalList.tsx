...
export default function EnquiryApprovalList({ enquiries }: { enquiries: PendingEnquiry[] }) {
  ...
  if (enquiries.length === 0) {
    return <p>No enquiries waiting for review.</p>;
  }
      ...
      <ul>
        {enquiries.map((e) => (
          <li key={e.id}>
            ...
            <br />
            From {e.buyer.name || e.buyer.email}: {e.message}
            <br />
            <button disabled={loadingId === e.id} onClick={() => handleDecision(e.id, "APPROVED")}>
              {loadingId === e.id ? "Working..." : "Approve"}
            </button>{" "}
            <button disabled={loadingId === e.id} onClick={() => handleDecision(e.id, "REJECTED")}>

      ...
      <ul>
        {properties.map((p) => (
          <li key={p.id}>
            ...
            — KSh {p.price.toLocaleString()} — Status: {p.status} — Tag:{" "}
            ...
            <br />
            <button disabled={loadingId === p.id} onClick={() => toggleVerified(p.id, p.verified)}>
              {loadingId === p.id ? "Working..." : p.verified ? "Mark Not Verified" : "Mark Verified"}
            </button>{" "}
            <button disabled={loadingId === p.id} onClick={() => toggleFeatured(p.id, p.featured)}>
              {loadingId === p.id ? "Working..." : p.featured ? "Unfeature" : "Feature"}
            </button>{" "}
            <button disabled={loadingId === p.id} onClick={() => toggleShowContact(p.id, p.showContact)}>
              {loadingId === p.id ? "Working..." : p.showContact ? "Hide contact from public" : "Show contact to public"}
            </button>{" "}
            <button disabled={loadingId === p.id} onClick={() => handleDelete(p.id, p.title)}>

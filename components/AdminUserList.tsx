      ...
      <ul>
        {users.map((u) => {
          ...
          return (
            <li key={u.id}>
              {label} — {u.email}
              {u.phone && <> — {u.phone}</>} — {u.role ? ROLE_LABELS[u.role] || u.role : "no role"}
              {canBeVerified && <> — {u.verified ? "Verified" : "Not Verified"}</>}
              {u.suspended && <> — SUSPENDED</>}
              {isSelf && <> (you)</>}
              <br />
              {isAdmin ? (
                <small>Admin accounts can't be suspended, deleted, or changed here.</small>
              ) : isSelf ? (
                <small>You can't suspend, delete, or change the role of your own account.</small>
              ) : (
                <>
                  <button disabled={loadingId === u.id} onClick={() => toggleSuspended(u.id, u.suspended)}>
                    {loadingId === u.id ? "Working..." : u.suspended ? "Unsuspend" : "Suspend"}
                  </button>{" "}
                  {canBeVerified && (
                    <>
                      <button disabled={loadingId === u.id} onClick={() => toggleVerified(u.id, u.verified)}>
                        {loadingId === u.id ? "Working..." : u.verified ? "Unverify" : "Verify"}
                      </button>{" "}
                    </>
                  )}
                  <button disabled={loadingId === u.id} onClick={() => handleDelete(u.id, label)}>

...
export default function PropertyApprovalList({ properties }: { properties: PendingProperty[] }) {
  ...
  if (properties.length === 0) {
    return <p>No listings waiting for review.</p>;
  }
  ...
  return (
    <div>
      ...
      <ul>
        {properties.map((p) => (
          <li key={p.id}>
            <strong>{p.title}</strong> — {getPropertyTypeLabel(p.propertyType, p.propertyTypeOther)} — {p.listingType} — KSh{" "}
            ...
            <small>
              ...
              {p.seller.verified && <> — Verified account</>}
            </small>
            {p.representingName && (
              <>
                <br />
            ...
            <br />
            <Link href={`/properties/${p.id}/documents`}>View supporting documents</Link>
            <br />
            <button disabled={loadingId === p.id} onClick={() => handleDecision(p.id, "APPROVED")}>
              {loadingId === p.id ? "Working..." : "Approve"}
            </button>{" "}
            <button disabled={loadingId === p.id} onClick={() => handleRequestChanges(p.id, p.title)}>
              {loadingId === p.id ? "Working..." : "Request changes"}
            </button>{" "}
            <button disabled={loadingId === p.id} onClick={() => handleDecision(p.id, "REJECTED")}>
